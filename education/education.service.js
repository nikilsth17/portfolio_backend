...
import { addEducationValidationSchema, paginationValidationSchema } from "./education.validation.js";
...
export const addEducation=async(req,res)=>{
    ...
    return res.status(201).send({message:"Education of user is added successfully."});
...
export const editEducation=async(req,res)=>{
    ...
    const isOwnerOfEducation= education.user &&education.user.equals(req.userInfo._id);
    if (isOwnerOfEducation){
        return res.status(403).send({message:"The information of education isnot belong to this user."});
    }

    //update education
    await Education.updateOne({_id:educationId},newValues);
    return res.status(200).send({message:"the education of user is successfully updated.."});
