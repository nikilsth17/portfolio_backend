...
export const registerUser=async(req,res)=>{
    ...
    if (user){
        return res.status(409).send({message:"User with this email address already exist."});
    }

    //hash password using bcrypt.hash()
    const hashedPassword= await bcrypt.hash(newUser.password,8);
    //replace newUser.password with hashedPassword
    newUser.password=hashedPassword;
    //create user on DB
    await User.create(newUser);
    
    //return response
    return res.status(200).send({message:"User is successflly registered."});
